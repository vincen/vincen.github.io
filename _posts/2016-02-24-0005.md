...
[环境参照](http://adspluz.com/000C/)，本机的 MySQL，内存占用400多 M，只是自己的测试环境，可以修改一下参数，使内存降到100M 以内。
...
UUID is defined based on [RFC 4122](https://tools.ietf.org/html/rfc4122). Two UUID values are expected to be distinct, even they are generated on two independent servers.
...
The `UUID()` function returns a UUID value in compliance with UUID version 1 described in the RFC 4122. For example:
...
### Advantages and disadvantages of using uuid as primary key
